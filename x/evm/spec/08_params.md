...
For example, on Ethereum, the `evm_denom` would be `ETH`. In the case of Ethermint, the default denomination is the **[atto jup](notion://www.notion.so/docs/basics/photon.md)** (used on the Evmos testnets). In terms of precision, the `JUP` and `ETH` share the same value, *i.e* `1 JUP = 10^18 atto jup` and `1 ETH = 10^18 wei`.
